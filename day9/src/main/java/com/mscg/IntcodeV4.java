...
public record IntcodeV4(long relativeBase, long[] data, Map<Long, Long> additionalData, long[] outputs, int ip, boolean halted)
{
	...
	public static IntcodeV4 parseInput(final BufferedReader in) throws IOException
	{
		final long[] data = Arrays.stream(in.readLine().split(",")) //
				.mapToLong(Long::parseLong) //
				.toArray();
		return new IntcodeV4(data);
	}

	public IntcodeV4(final long[] data) {
		this(0, data, Map.of(), new long[0], 0, false);
	}

	public IntcodeV4 withNounAndVerb(final int noun, final int verb)
	{
		final long[] data = this.data.clone();
		data[1] = noun;
		data[2] = verb;
		return new IntcodeV4(relativeBase, data, additionalData, outputs, ip, halted);
	}

	public IntcodeV4 execute(final Iterator<Long> inputs, final boolean interruptOnOutput)
	{
	...
	public IntcodeV4 execute(final Deque<Long> inputs, final boolean interruptOnOutput)
	{
		if (halted) {
		...
		return new IntcodeV4(data.relativeBase(), data.data(), Map.copyOf(data.additionalData()), outputsArr, lastIp, halted);
	}
