...
public record TowelsPatterns(List<String> availablePatterns, List<String> desiredPatterns)
{
	...
	{
		record Node(String originalPattern, List<String> availablePatterns, String desiredPattern, List<String> usedPatterns)
		{
			@Override
		...
		final BfsVisitor<Node, String, Node> bfsVisitor = BfsVisitor.<Node, String, Node>builder() //
				...
				.withSimpleAdjacentMapper( //
						node -> node.availablePatterns().stream() //
								.filter(pattern -> node.desiredPattern().startsWith(pattern)) //
								.sorted(Comparator.comparingInt(String::length)) //
								.map(pattern -> new Node(node.originalPattern(), node.availablePatterns(),
										node.desiredPattern().substring(pattern.length()),
		...
		for (final String desiredPattern : desiredPatterns) {
			final VisitResult<Node> result = bfsVisitor.visitFrom(
					new Node(desiredPattern, availablePatterns, desiredPattern, List.of()), BfsVisitor.VisitMode.DFS);
