...
public record IntcodeV5(long relativeBase, long[] data, Map<Long, Long> additionalData, long[] outputs, int ip, boolean halted)
{
	...
	public static IntcodeV5 parseInput(final BufferedReader in) throws IOException
	{
		final long[] data = Arrays.stream(in.readLine().split(",")) //
				.mapToLong(Long::parseLong) //
				.toArray();
		return new IntcodeV5(data);
	}

	public IntcodeV5(final long[] data) {
		this(0, data, Map.of(), new long[0], 0, false);
	}

	public IntcodeV5 withNounAndVerb(final int noun, final int verb)
	{
		final long[] data = this.data.clone();
		data[1] = noun;
		data[2] = verb;
		return new IntcodeV5(relativeBase, data, additionalData, outputs, ip, halted);
	}

	public IntcodeV5 execute(final Iterator<Long> inputs)
	{
		return execute(inputs, Integer.MAX_VALUE);
	}

	public IntcodeV5 execute(final Iterator<Long> inputs, final int outputsBeforeInterrupt)
	{
		final Deque<Long> inputsQueue = Seq.seq(inputs).collect(Collectors.toCollection(LinkedList::new));
		return execute(inputsQueue, outputsBeforeInterrupt);
	}

	public IntcodeV5 execute(final Deque<Long> inputs)
	{
		return execute(inputs, Integer.MAX_VALUE);
	}

	public IntcodeV5 execute(final Deque<Long> inputs, final int outputsBeforeInterrupt)
	{
		if (halted) {
		...
		return new IntcodeV5(data.relativeBase(), data.data(), Map.copyOf(data.additionalData()), outputsArr, lastIp, halted);
	}
